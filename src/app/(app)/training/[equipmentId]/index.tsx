...
import { AnnotationToolbar, CTAButton, CheckBadge, Chip, Loading, ProgressRing, toast, type AnnotationTool, type CameraBackdropHandle, type Stroke } from '@/shared/components';
...
export default function TrainingScreen() {
  const { equipmentId, demo, part: initialPart, tab: initialTab } = useLocalSearchParams<{ equipmentId: string; demo?: string; part?: string; tab?: Tab }>();
  const { t, tr } = useI18n();
  ...
  const [selected, setSelected] = useState<EquipmentPart | undefined>();
  ...
  useEffect(() => {
    if (initialPart && equipment.data) setSelected(equipment.data.parts.find((p) => p.id === initialPart));
  }, [initialPart, equipment.data]);
  ...
  const startMode = (m: Exclude<Mode, null>) => {
    ...
    setPanel(m === 'free');
    const s: TrainingSession = { id: `ts-${Date.now()}`, userId: user.id, equipmentId: eq.id, procedureId: m === 'step' ? procedure?.id : undefined, mode: m, completedStepIds: [], inspectedPartIds: [], startedAt: new Date().toISOString() };
    setSession(s);
  ...
  const completeStep = () => {
    ...
    const next: TrainingSession = { ...session, completedStepIds: [...session.completedStepIds, current.id], finishedAt: last ? new Date().toISOString() : undefined };
    setSession(next);
    saveSession.mutate(next);
    if (last) {
  ...
  const percent = mode === 'step' && procedure ? Math.round(((session?.completedStepIds.length ?? 0) / procedure.steps.length) * 100) : Math.round(((session?.inspectedPartIds.length ?? 0) / eq.parts.length) * 100);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.white }}>
      ...
      <View style={[styles.tabs, { paddingTop: insets.top }]}>
        <Pressable style={[styles.tab, tab === 'training' ? styles.tabOn : styles.tabOff]} onPress={() => setTab('training')} accessibilityRole="tab" accessibilityState={{ selected: tab === 'training' }}>
        ...
        <Pressable testID="tab-assistance" style={[styles.tab, tab === 'assistance' ? styles.tabOn : styles.tabOff]} onPress={() => setTab('assistance')} accessibilityRole="tab" accessibilityState={{ selected: tab === 'assistance' }}>
      ...
      <View style={styles.sub}>
        ...
        {tab === 'assistance' && (
          <View style={styles.subIcons}>
            <Pressable onPress={() => router.push({ pathname: '/training/[equipmentId]/alerts', params: { equipmentId: eq.id } })} accessibilityLabel={t('assistance.alerts')} hitSlop={6}>
      ...
      <View style={{ flex: 1 }}>
        ...
        {tab === 'assistance' && (
          <View style={styles.quick}>
            <QuickAction icon="alert-circle" color={colors.red} label={t('assistance.alerts')} badge={openAlerts.length} onPress={() => router.push({ pathname: '/training/[equipmentId]/alerts', params: { equipmentId: eq.id } })} testID="qa-alerts" />
            <QuickAction icon="time" label={t('assistance.history')} onPress={() => router.push({ pathname: '/training/[equipmentId]/history', params: { equipmentId: eq.id } })} testID="qa-history" />
            <QuickAction icon="people" label={t('assistance.notifications')} onPress={() => router.push({ pathname: '/training/[equipmentId]/notifications', params: { equipmentId: eq.id } })} testID="qa-notifications" />
            <QuickAction icon="videocam" color={colors.green} label={t('assistance.call')} onPress={() => router.push('/call')} testID="qa-call" />
          </View>
        )}
          ...
          <View style={styles.stepCard}>
            ...
            <View style={styles.stepBody}>
              ...
              <View style={{ alignItems: 'center', gap: 6 }}>
                ...
                <CheckBadge size={44} done={false} onPress={selected?.id === current.partId ? completeStep : () => toast(t('training.tapPart'))} />
              </View>
            </View>
          </View>
        )}
        ...
        {tab === 'training' && mode && (
          <Pressable style={styles.exit} onPress={() => { setMode(null); setPanel(false); }} accessibilityRole="button" accessibilityLabel={t('training.exit')}>
        ...
        {tab === 'training' && !panel && (
          <Pressable style={styles.handle} onPress={() => setPanel(true)} accessibilityRole="button" accessibilityLabel={t('training.experience')} />
        )}
        {tab === 'training' && panel && (
          <PartPanel equipment={eq} part={selected} percent={percent} done={!!selected && !!session?.inspectedPartIds.includes(selected.id)} onSelect={onPart} onClose={() => setPanel(false)} />
        )}
      </View>
...
function QuickAction({ icon, label, onPress, color = colors.primary, badge, testID }: { icon: keyof typeof Ionicons.glyphMap; label: string; onPress: () => void; color?: string; badge?: number; testID?: string }) {
  return (
...
const styles = StyleSheet.create({
  ...
  sub: { backgroundColor: colors.primary, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 6, borderTopWidth: 1, borderColor: 'rgba(255,255,255,0.3)' },
  ...
  quick: { position: 'absolute', top: spacing.xl + 8, left: spacing.sm, right: spacing.sm, flexDirection: 'row', justifyContent: 'space-between', gap: spacing.xs },
  qa: { flex: 1, alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.92)', borderRadius: radius.md, paddingVertical: spacing.sm, paddingHorizontal: 2, gap: 4 },
  ...
  badge: { position: 'absolute', top: -4, right: -6, backgroundColor: colors.navy, borderRadius: 9, minWidth: 18, height: 18, alignItems: 'center', justifyContent: 'center', borderWidth: 1.5, borderColor: colors.white },
  ...
  modeHint: { alignSelf: 'center', backgroundColor: 'rgba(6,32,91,0.75)', color: colors.white, fontFamily: fonts.medium, fontSize: 12, paddingHorizontal: spacing.md, paddingVertical: 4, borderRadius: radius.pill, overflow: 'hidden' },
  ...
  stepCard: { position: 'absolute', left: 0, right: 0, bottom: 0, backgroundColor: colors.white, borderTopLeftRadius: radius.md, borderTopRightRadius: radius.md, overflow: 'hidden' },
  ...
  exit: { position: 'absolute', top: spacing.sm, right: spacing.sm, flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: radius.pill, paddingHorizontal: 10, paddingVertical: 4 },
  exitText: { color: colors.white, fontFamily: fonts.medium, fontSize: 11 },
  handle: { position: 'absolute', right: 0, top: '38%', width: 12, height: 80, backgroundColor: colors.primary, borderTopLeftRadius: 6, borderBottomLeftRadius: 6 },
});
