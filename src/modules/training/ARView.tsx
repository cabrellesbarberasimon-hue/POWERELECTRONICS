 ...
 */
export function ARView({ equipment, hotspots, onPartPress, selectedPartId, drawing, strokes = [], onStrokes, rotate3d, demo, cameraRef, overlay }: ARViewProps) {
  const { t, tr } = useI18n();
  ...
  useEffect(() => {
    const s = Animated.loop(Animated.timing(scan, { toValue: 1, duration: 1100, easing: Easing.inOut(Easing.quad), useNativeDriver: true }));
    s.start();
  ...
  const rotator = useRef(
    PanResponder.create({
      ...
      onPanResponderGrant: () => {
        angleStart.current = angleRef.current;
      },
      onPanResponderMove: (_, g) => setAngle(Math.max(-55, Math.min(55, angleStart.current + g.dx / 3))),
    }),
  ).current;
  ...
  return (
    <View style={styles.root} onLayout={(e) => setSize({ w: e.nativeEvent.layout.width, h: e.nativeEvent.layout.height })}>
      <CameraBackdrop ref={cameraRef} demo={demo} onModeChange={setLive} fallback={<View style={[StyleSheet.absoluteFill, { backgroundColor: '#E9EDF1' }]} />} />

      {size.w > 0 && detected && (
        <View
          style={{ position: 'absolute', left, top, width: cabW, height: cabH, opacity: live ? 0.88 : 1, transform: [{ perspective: 900 }, { rotateY: `${angle}deg` }] }}
              ...
              >
                {(h.pulse || selected) && (
                  <Animated.View
                    style={[
                      styles.ring,
                      { borderColor: color, opacity: pulse.interpolate({ inputRange: [0, 1], outputRange: [0.9, 0.1] }), transform: [{ scale: pulse.interpolate({ inputRange: [0, 1], outputRange: [1, 2.2] }) }] },
                    ]}
                  />
                )}
      ...
      {!detected && (
        <View style={StyleSheet.absoluteFill} pointerEvents="none">
          <Animated.View style={[styles.scanLine, { transform: [{ translateY: scan.interpolate({ inputRange: [0, 1], outputRange: [size.h * 0.2, size.h * 0.8] }) }] }]} />
      ...
      {detected && (
        <View style={styles.detected} pointerEvents="none">
          <Text style={styles.detectedText}>
            {live ? `◉ ${t('training.detected')}` : `◎ ${t('training.cameraOff')}`}
          </Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  scanLine: { position: 'absolute', left: '8%', right: '8%', height: 3, backgroundColor: colors.primary, shadowColor: colors.primary, shadowOpacity: 0.8, shadowRadius: 8 },
  scanBadge: { position: 'absolute', top: spacing.lg, alignSelf: 'center', backgroundColor: 'rgba(6,32,91,0.75)', borderRadius: 20, paddingHorizontal: spacing.md, paddingVertical: 6 },
  scanText: { color: colors.white, fontFamily: fonts.medium, fontSize: 12 },
  detected: { position: 'absolute', top: spacing.sm, left: spacing.sm, backgroundColor: 'rgba(255,255,255,0.85)', borderRadius: 12, paddingHorizontal: 8, paddingVertical: 3 },
